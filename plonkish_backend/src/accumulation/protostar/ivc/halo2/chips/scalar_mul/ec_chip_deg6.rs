    ...
    pub fn configure(meta: &mut ConstraintSystem<C::Scalar>, advices: [Column<Advice>; NUM_ADVICE]) -> Self {
        ...
        let [q_ec_double_add_odd, q_ec_double_add_even] = [(); NUM_SELECTOR].map(|_| meta.selector());;
            ...
            meta.create_gate("q_ec_double_add_odd", |meta| {
                ...
                let lambda_next = meta.query_advice(col_lambda, Rotation(1));
                ...
                //     * ( - sel_y.clone() * acc_next_y.clone()) - three.clone() * b.clone() * sel_z.clone()* acc_next_z.clone()
                //     - ( sel_y.clone() * acc_next_z.clone() - acc_next_y.clone() * sel_z.clone())
                //     * (three.clone() * b.clone() * (sel_x.clone() * acc_next_z.clone() + acc_next_x.clone() * sel_z.clone()));
                ...
                // let acc_sub_z = (sel_y.clone() * acc_next_z.clone() - acc_next_y.clone() * sel_z.clone())
                //     * ( - sel_y.clone() * acc_next_y.clone() + three.clone() * b.clone() * sel_z.clone() * acc_next_z.clone())
            ...
            let acc_sub_z = (sel_y.clone()  - acc_next_y.clone() * sel_z.clone())
                ...
                * (three.clone() * sel_x.clone() * acc_next_x.clone());
                ...
                // vec![q_ec_double_add.clone() * ( - acc_double_x.clone() * lambda.clone()),
                //      q_ec_double_add.clone() * ( - acc_double_y.clone() * lambda.clone()),
                //      q_ec_double_add * ( - acc_double_z.clone() * lambda.clone())]
            });
