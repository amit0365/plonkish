...
use crate::accumulation::protostar::ivc::halo2::chips::sha256::table16::{
...
impl<F:PrimeField> CompressionConfig<F> {
    ...
    pub(super) fn assign_h_prime(
        ...
        {
            AssignedBits::<16,F>::assign_bits(region, || "k_lo", a_6, row - 1, Value::known(k_lo))?;
        ...
        // Assign h_prime_lo, h_prime_hi, h_prime_carry
        {
            ...
            let h_prime_lo =
                AssignedBits::<16,F>::assign_bits(region, || "h_prime_lo", a_7, row + 1, h_prime_lo)?;
            let h_prime_hi =
                AssignedBits::<16,F>::assign_bits(region, || "h_prime_hi", a_8, row + 1, h_prime_hi)?;
