...
impl<T> ColumnQuery<T> {
    ...
    pub fn row_idx(&self, idx: usize, num_vars: usize) -> usize {
...
pub trait QueryType {
    ...
    type F: Field;
    // ChallengeVariable
    type Challenge: Copy + PartialEq + Debug;
    // FixedVariable
    ...
    fn linear_combination_constraints(
        constraints: &[QueriedExpression<Self>],
        ys: &[QueriedExpression<Self>],
    ) -> QueriedExpression<Self> {
        assert_eq!(constraints.len() - 1, ys.len());
        let products = constraints
            .iter()
            .skip(1)
            .zip(ys.iter())
            .map(|(lhs, rhs)| lhs.clone() * rhs.clone());
