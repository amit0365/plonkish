    ...
    C::Base: BigPrimeField + PrimeFieldBits,
{
    pub fn configure(meta: &mut ConstraintSystem<C::Scalar>, advices: [Column<Advice>; NUM_ADVICE]) -> Self {
        ...
        meta.create_gate("q_ec_double_add_even", |meta| {
            ...
            let r = meta.query_advice(col_rbits, Rotation(1));
            let x = meta.query_advice(col_pt, Rotation(0));
            let z = meta.query_advice(col_pt, Rotation(1));
            let acc_x = meta.query_advice(col_acc_x, Rotation(0));
            let acc_z = meta.query_advice(col_acc_y, Rotation(0));
            // let acc_y = Expression::Constant(C::Scalar::ONE);

            let acc_next_x = meta.query_advice(col_acc_x, Rotation(1));
            let acc_next_z = meta.query_advice(col_acc_y, Rotation(1));
            // let acc_next_y = Expression::Constant(C::Scalar::ONE);
            let lambda = meta.query_advice(col_lambda, Rotation(1));
            ...
            // simplified for b = 3, 
            let acc_double_x = two * acc_x.clone() * (one.clone() - twenty_seven.clone() * acc_z_sq.clone());
            let acc_double_y = (one.clone() - twenty_seven.clone() * acc_z_sq.clone()) * (one.clone() 
                                + nine.clone() * acc_z_sq.clone()) + seventy_two.clone() * acc_z_sq.clone();
            let acc_double_z = eight.clone() * acc_z.clone();

            // choice poly in projective coordinates, identity is (0,1,0)
            let sel_x = r.clone() * x.clone(); 
            let sel_y = one.clone(); 
            let sel_z = r.clone() * z.clone(); 
        ...
        // simplified for b = 3, 
        let acc_sub_x = ( - acc_next_x.clone() + sel_x.clone())
            * ( - one.clone() - nine.clone() * sel_z.clone() * acc_next_z.clone())
            - ( acc_next_z.clone()  - sel_z.clone())
            * (nine.clone() * (sel_x.clone() * acc_next_z.clone() + acc_next_x.clone() * sel_z.clone()));
        ...
        let acc_sub_z = (sel_z.clone() - acc_next_z.clone())
            * ( - one.clone() + nine.clone() * sel_z.clone() * acc_next_z.clone())
            + ( - acc_next_x.clone() + sel_x.clone())
            * (three.clone() * acc_next_x.clone() * sel_x.clone());
...
mod test {
    ...
    fn ec_vec() {
        ...
        rbits.extend( [true, false, false, true]);
        ...
        // for i in (0..vec_len) {
        //     let lhs = G1::double(&acc_prev.into());
        //     acc_prev = lhs.to_affine();
        //     println!("rbit: {}", rbits_rev[i]);

        //     acc_prev = if rbits_rev[i] { (acc_prev + p).to_affine() } else { (acc_prev + zero).to_affine() };
        //     acc_x_vec.push(Value::known(acc_prev.x));
        //     acc_y_vec.push(Value::known(acc_prev.y)); 

        //     let rhs = if rbits_rev[i] { acc_prev - p } else { acc_prev - zero};
        ...
        for i in 0..vec_len {
            if i % 2 == 0 {
                pt_vec.push(Value::known(p.x*p_yinv));
                pt_vec.push(Value::known(p.z*p_yinv));
            }
        }
        ...
        // let scalar_mul_given = (p * r).to_affine();
        // let scalar_mul_calc = G1Affine::from_xy(acc_x_vec.last().map(|val| val.assign().unwrap()).unwrap(), acc_z_vec.last().map(|val| val.assign().unwrap()).unwrap()).unwrap();
