use halo2_base::{gates::flex_gate::{FlexGateConfig, FlexGateConfigParams}, halo2_proofs::
    {circuit::{AssignedCell, Layouter, SimpleFloorPlanner, Value}, halo2curves::{bn256::{G1Affine, G2Affine, G1}, grumpkin::{self, Fr as Fq}}, plonk::{Advice, Assigned, Circuit, Column, ConstraintSystem, Constraints, Error, Expression, Fixed, Instance, Selector}, poly::Rotation
    ...
    C::Base: BigPrimeField + PrimeFieldBits,
{
    ...
    pub fn assign(
        ...
        // | 129 |    -      |   2^128   |    r      |  comm_x   |  comm_y   |    sm.X   |    sm.Y   |     X3    |    Y3     |
                ...
                let r = region.assign_advice(|| "r", self.witness[1], last_row, || inputs.rlc_vec[last_row - 1])?;
                let nark_x = region.assign_advice(|| "nark_X", self.witness[2], 1, || inputs.ptx_vec[0])?;
                let nark_y = region.assign_advice(|| "nark_Y", self.witness[3], 1, || inputs.pty_vec[0])?;
                let acc_x = region.assign_advice(|| "comm_X", self.witness[2], last_row, || inputs.comm_X)?;
                let acc_y = region.assign_advice(|| "comm_Y", self.witness[3], last_row, || inputs.comm_Y)?;
