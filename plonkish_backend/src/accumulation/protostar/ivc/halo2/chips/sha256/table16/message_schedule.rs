...
#[derive(Clone, Debug)]
pub(super) struct MessageWord<F:PrimeField>(AssignedBits<32,F>);
...
#[derive(Clone, Debug)]
pub(super) struct MessageScheduleConfig<F> {
    lookup: SpreadInputs,
...
impl<F: PrimeField> Table16Assignment<F> for MessageScheduleConfig<F> {}
