...
impl<F:PrimeField> MessageScheduleConfig<F> {
    ...
    pub fn assign_word_and_halves(
        ...
        word: Value<u32>,
        word_idx: usize,
    ) -> Result<(AssignedBits<32,F>, (AssignedBits<16,F>, AssignedBits<16,F>)), Error> {
        ...
        let w_lo = {
            ...
            AssignedBits::<16,F>::assign(region, || format!("W_{}_lo", word_idx), a_3, row, w_lo_val)?
        };
        let w_hi = {
            let w_hi_val = word.map(|word| (word >> 16) as u16);
            AssignedBits::<16,F>::assign(region, || format!("W_{}_hi", word_idx), a_4, row, w_hi_val)?
        };
