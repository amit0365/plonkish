...
pub use halo2_base::halo2_proofs::halo2curves::CurveAffine;
...
impl<'a, F: PrimeField> Paired<'a, F> {
        ...
        Pcs: PolynomialCommitmentScheme<F, Polynomial = MultilinearPolynomial<F>>,
    {
        ...
        // let ys = std::iter::once([&F::ONE, &F::ONE])
        // .chain(total_challenges.iter().rev().take(num_alpha_primes).cloned().rev())
        // .collect_vec();
        let ys = total_challenges.iter().rev().take(num_alpha_primes).cloned().rev().collect_vec();

        let lookups_polys = if lookups_empty {
            Vec::new()
        } else { 
            total_advice[num_witness_polys..num_witness_polys + 3].to_vec()
        };
        ...
        //         .iter()
        //         .map(|(lookup)| {
        //             let m = [&lookup0[0], &lookup1[1]];
        //             let g = [&lookup0[1], &lookup1[1]];
        //             let h = [&lookup0[2], &lookup1[1]];
