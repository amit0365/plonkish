...
#[derive(Clone, Debug)]
pub struct AssignedBits<const LEN: usize, F:PrimeField>(AssignedCell<Bits<LEN>, F>);

impl<const LEN: usize, F:PrimeField> std::ops::Deref for AssignedBits<LEN,F> {
...
impl<F:PrimeField> Table16Chip<F> {
    ...
    pub fn configure(meta: &mut ConstraintSystem<F>, advice: [Column<Advice>; 7]) -> <Self as Chip<F>>::Config {
        // Columns required by this chip:
        let message_schedule = advice[0];
        ...
        let compression =
            CompressionConfig::<F>::configure(meta, lookup_inputs.clone(), message_schedule, extras);

        let message_schedule =
            MessageScheduleConfig::<F>::configure(meta, lookup_inputs, message_schedule, extras);
    ...
    pub fn load(config: Table16Config<F>, layouter: &mut impl Layouter<F>) -> Result<(), Error> {
        SpreadTableChip::<F>::load(config.lookup, layouter)
    }
}
    ...
    fn print_sha256_circuit() {
        ...
        impl Circuit<Fr> for MyCircuit {
            ...
            fn synthesize(
                ...
                Sha256::digest(table16_chip, &mut layouter, &input)?;
        ...
        let root =
            BitMapBackend::new("sha-256-table16-chip-layout.png", (5024, 12480)).into_drawing_area();
        ...
        //     .render::<Fr, _, _>(17, &circuit, &root).unwrap();
    }
}