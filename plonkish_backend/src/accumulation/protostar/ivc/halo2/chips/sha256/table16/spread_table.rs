...
impl<F:PrimeField,const DENSE: usize, const SPREAD: usize> SpreadVar<F,DENSE, SPREAD> {
    pub(super) fn with_lookup(
        ...
        let spread =
            AssignedBits::<SPREAD,F>::assign_bits(region, || "spread", cols.spread, row, spread_val)?;
...
mod tests {
    ...
    use halo2_proofs::{
        circuit::{Layouter, SimpleFloorPlanner, Value}, dev::MockProver, halo2curves::pasta::Fp, plonk::{Advice, Circuit, Column, ConstraintSystem, Error}
    };
